...
import { set } from "zod"; // This is unused and can be removed, but kept for context
...
        // Data is now expected to be { en: [...], ru: [...] }
        setAllTransportData(data);
      } catch (error) {
...
    return allTransportData[currentLang] || allTransportData['en'] || [];
...
    
    // Try to get data for the current language, fallback to English
    return allHotelData[currentLang] || allHotelData['en'] || [];
  }, [allHotelData, currentLang]);

  // --- SCROLL & PANEL LOGIC (Remains largely the same) ---
...
  // --- COMPONENTS (Kept in-line as per original) ---
...
                        // FEATURES/FACILITIES ARE NOW EXPECTED TO BE LOCALIZED STRINGS IN THE JSON
                        <li key={i}>{feature}</li>
                      ))}
                    </ul>
                  </div>
...
          ) : (
            <motion.div
                key={`transport-${currentLang}`} // Key ensures re-animation on language switch
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
...
          ) : (
            <motion.div
                key={`hotels-${currentLang}`} // Key ensures re-animation on language switch
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ duration: 0.5 }}
              ref={hotelsRef}
...
                hotelServices.map((h, idx) => (
    ...
                        }
                        tag={
                          h.location?.city || // Use optional chaining for safe access
...
          </div>
        </div>
        {/* Slide-in panel (Unchanged logic, just using localized strings) */} 
        <SlideInPanel
          isOpen={panelOpen}
          onClose={closePanel}
